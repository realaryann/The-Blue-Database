...
def list_table(dbname,frame):
    returnlist = []
    testname =f'{dbname}'
    tcon = sqlite3.connect(testname)
    tcur = tcon.cursor()
    query="SELECT name from sqlite_master where type = 'table';"
    tcur.execute(query)
    templist = tcur.fetchall()
    for i in templist:
        for j in i:
            returnlist.append(j)
    tabelslabel = Label(frame, text="Table List",bg="slategrey",padx=30,pady=10)
    tabelslabel.configure(font=("Modern",15))
    tabelslabel.grid(row = 3, column =1)
    out = Text(frame,height=10,width=30,pady=20,bg="lightblue3")
    out.configure(font=("Modern",15))
...
def get_all_tables():
    gettab = Toplevel()
    gettab.configure(bg="lightblue3")
    gettab.title("Get Tables")
    gettab.iconbitmap("favicon.ico")
    tabhead = Label(gettab, text="Get List of Tables",bg="slategrey",padx=40,pady=10)
    tabhead.configure(font=("modern",30))
    tabhead.pack()
    frameit = LabelFrame(gettab,padx=100,pady=50,bg="lightblue3")
    frameit.pack()
    enterdata = Label(frameit,text="Enter the Database Name",bg="slategrey",padx=50,pady=10)
    enterdata.configure(font=("Modern",20))
    enterdata.grid(row = 0, column = 1,pady=10)
    tabentry = Entry(frameit, borderwidth=4,width=40)
    tabentry.grid(row = 1, column = 1)
    tabsubmit = Button(frameit,text="Submit",padx=20,pady=10,bg="slategrey",command = lambda:list_table(tabentry.get(),frameit))
    tabsubmit.configure(font=("Modern",15))
    tabsubmit.grid(row = 2, column =1,pady=20)
    gettab.resizable(FALSE,FALSE)
...
frame1 = LabelFrame(root,padx=200,pady=100,bg="slategrey")
...
get_tables = Button(frame1, text="Get Table List",bg= "lightblue3",padx= 117, pady=40,command = get_all_tables)
get_tables.pack(pady=20)
get_tables.config(font=("Modern",15))
